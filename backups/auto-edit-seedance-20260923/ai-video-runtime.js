/* Hotel video editor. Seedance generates the visual source video and Turing
   Audio generates the editable narration; the local renderer adds captions
   and motion graphics. */
(function () {
  'use strict';
  ...
  const voiceProfiles = {
    verse: { label: 'Verse · 自然叙述', apiVoice: 'verse', speed: 1 },
    coral: { label: 'Coral · 温和女声', apiVoice: 'coral', speed: .98 },
    shimmer: { label: 'Shimmer · 明亮女声', apiVoice: 'shimmer', speed: 1.02 },
    sage: { label: 'Sage · 稳重女声', apiVoice: 'sage', speed: .96 },
    alloy: { label: 'Alloy · 清晰中性', apiVoice: 'alloy', speed: 1 },
    ash: { label: 'Ash · 沉稳男声', apiVoice: 'ash', speed: .96 },
    echo: { label: 'Echo · 低沉男声', apiVoice: 'echo', speed: .92 },
    ballad: { label: 'Ballad · 温和叙述', apiVoice: 'ballad', speed: .98 }
  };
  ...
  function updateAssetTagUi(index) {
    ...
    if (badge) {
      ...
      badge.textContent = asset.tagSource === 'manual' ? '手动' : '未选择';
      badge.title = asset.rationale || '';
    }
  }
  ...
  function selectedSceneLabels() {
    return [...new Set(state.assets.filter((asset) => asset.tagSource === 'manual' && asset.sceneType).map((asset) => sceneMeta(asset.sceneType).label))];
  }
  ...
  function renderHeadlinePreview(node, text, sceneType) {
    if (!node) return;
    const parts = headlineParts(text, sceneType);
    const estimatedWidth = (parts.lead.length + parts.tail.length) * 13 + parts.keyword.length * 27 + 12;
    node.style.setProperty('--headline-fit', String(Math.max(.68, Math.min(1, 270 / Math.max(1, estimatedWidth))).toFixed(3)));
    const line = document.createElement('span'); line.className = 'ai-video-headline-line';
    const append = (value, className) => {
      if (!value) return;
      const span = document.createElement('span'); span.className = className; span.textContent = value; line.appendChild(span);
    };
    append(parts.lead, 'ai-video-headline-support');
    append(parts.keyword, 'ai-video-headline-keyword');
    append(parts.tail, 'ai-video-headline-support ai-video-headline-support-tail');
    const rule = document.createElement('span'); rule.className = 'ai-video-headline-rule'; rule.setAttribute('aria-hidden', 'true');
    node.replaceChildren(line, rule);
  }
  ...
  function addFiles(files) {
    ...
    renderAssets();
    syncControls();
    setStatus(`已添加 ${state.assets.length} 个素材，可按需手动选择镜头标签。`, 'ready');
  }
  ...
  function renderAssets() {
    ...
    state.assets.forEach((asset, index) => {
      ...
      detection.textContent = asset.tagSource === 'manual' ? '手动' : '未选择';
      ...
      sceneSelect.addEventListener('change', () => {
        ...
        asset.manualOverride = true; asset.tagSource = 'manual'; asset.confidence = 1; updateAssetTagUi(index);
  ...
  function buildCopyVariants() {
    ...
    const topic = String($('#aiVideoTopic')?.value || '').trim();
    const highlights = String($('#aiVideoHighlights')?.value || '').trim().replace(/[，,；;]/g, '、');
    const sceneLabels = selectedSceneLabels();
    const sceneHint = sceneLabels.length ? `画面重点为${sceneLabels.join('、')}，` : '';
    const taggedHighlights = sceneLabels.length ? `${highlights}、${sceneLabels.join('、')}` : highlights;
    const audience = String($('#aiVideoAudience')?.value || '').trim();
    const offer = String($('#aiVideoOffer')?.value || '').trim();
    ...
    return [
      `${topic}，把旅途里的疲惫交给一间舒服的房间。${sceneHint}这里有${taggedHighlights}，从入住到离店，每一个细节都让人更安心。${audienceLine}${offerLine}`,
      `周末想换个地方放松？来住${topic}。${sceneHint}${taggedHighlights}，让每一次入住都多一点从容。${audienceLine}${offerLine}`,
      `这次出发，住得舒服也很重要。${topic}${sceneLabels.length ? `，从${sceneLabels.join('、')}到贴心服务` : `用${highlights}` }，把旅途的舒适感安排到每个细节。${audienceLine}${offerLine}`,
      `把假期过成喜欢的样子：${topic}。${sceneHint}从${taggedHighlights}到贴心服务，推开门就是轻松的开始。${audienceLine}${offerLine}`,
      `正在计划下一次入住？${topic}值得收藏。${sceneHint}${taggedHighlights}，给忙碌旅程留一段真正放松的时间。${audienceLine}${offerLine}`
    ];
  }
  ...
  function createSegments() {
    ...
    const pickAssetForText = (text, index) => {
      ...
      const matching = state.assets.map((asset, assetIndex) => ({ asset, assetIndex })).filter(({ asset, assetIndex }) => {
        const trustedType = asset.tagSource === 'manual' && asset.sceneType ? asset.sceneType : '';
  ...
  function segmentDuration(segment) {
    return Math.max(.1, Math.max(segment.mediaEnd - segment.mediaStart, segment.captionEnd - segment.captionStart));
  }
  ...
  function renderPreviewAt(ratio = 0) {
    ...
    const caption = $('#aiVideoPreviewCaption');
    const captionText = $('#aiVideoPreviewCaptionText') || caption;
    captionText.textContent = (item.text || '酒店好住，也要好看').slice(0, 42);
    caption.hidden = !state.subtitles;
    caption.dataset.style = state.subtitleStyle;
    if (segmentChanged) { caption.classList.remove('is-entering'); void caption.offsetWidth; caption.classList.add('is-entering'); }
    const emphasis = $('#aiVideoPreviewEmphasis');
    if (emphasis) {
      emphasis.dataset.style = state.subtitleStyle;
      renderHeadlinePreview(emphasis, item.text, asset.sceneType || 'other');
      if (segmentChanged) { emphasis.classList.remove('is-popping'); void emphasis.offsetWidth; emphasis.classList.add('is-popping'); }
    }
  ...
  function drawEmphasis(ctx, text, sceneType, width, height, progress = 1) {
    const style = subtitleStyles[state.subtitleStyle] || subtitleStyles['clean-white'];
    const parts = headlineParts(text, sceneType);
    const unit = Math.min(width, height);
    let supportSize = Math.round(unit * .046);
    let keywordSize = Math.round(unit * .082);
    const maxWidth = width * .86;
    const measureLine = () => {
      ctx.font = `800 ${supportSize}px "PingFang SC", sans-serif`; const leadWidth = ctx.measureText(parts.lead).width;
      const tailWidth = ctx.measureText(parts.tail).width;
      ctx.font = `950 ${keywordSize}px "PingFang SC", sans-serif`; const keywordWidth = ctx.measureText(parts.keyword).width;
      return { leadWidth, tailWidth, keywordWidth, total: leadWidth + tailWidth + keywordWidth + unit * .025 };
    };
    let widths = measureLine();
    if (widths.total > maxWidth) {
      const fit = maxWidth / widths.total;
      supportSize = Math.max(22, Math.floor(supportSize * fit));
      keywordSize = Math.max(38, Math.floor(keywordSize * fit));
      widths = measureLine();
    }
    const scale = .92 + Math.min(1, progress * 5) * .08;
    const centerX = width / 2; const baselineY = height * .255;
    ctx.save(); ctx.globalAlpha = Math.min(1, progress * 5); ctx.translate(centerX, baselineY); ctx.scale(scale, scale); ctx.textAlign = 'left';
    ctx.shadowColor = 'rgba(2,12,27,.78)'; ctx.shadowBlur = Math.round(unit * .018); ctx.shadowOffsetY = Math.round(unit * .004);
    const startX = -widths.total / 2;
    ctx.fillStyle = 'rgba(255,255,255,.94)'; ctx.font = `800 ${supportSize}px "PingFang SC", sans-serif`;
    ctx.fillText(parts.lead, startX, 0);
    const keywordX = startX + widths.leadWidth + unit * .012;
    ctx.font = `950 ${keywordSize}px "PingFang SC", sans-serif`; ctx.lineJoin = 'round'; ctx.lineWidth = Math.max(1.5, unit * .0035); ctx.strokeStyle = style.stroke;
    ctx.strokeText(parts.keyword, keywordX, 0); ctx.fillStyle = style.accent; ctx.fillText(parts.keyword, keywordX, 0);
    const tailX = keywordX + widths.keywordWidth + unit * .012;
    ctx.fillStyle = 'rgba(255,255,255,.94)'; ctx.font = `800 ${supportSize}px "PingFang SC", sans-serif`; ctx.fillText(parts.tail, tailX, 0);
    ctx.strokeStyle = style.stroke; ctx.globalAlpha *= .8; ctx.lineWidth = Math.max(1, unit * .0015); ctx.beginPath(); ctx.moveTo(-unit * .16, keywordSize * .64); ctx.lineTo(unit * .16, keywordSize * .64); ctx.stroke();
    ctx.restore();
  }
  ...
  function createMusicBuffer(context, seconds, style) {
    ...
    for (let index = 0; index < length; index += 1) {
      ...
      let value = chord.reduce((sum, frequency, noteIndex) => sum + Math.sin(2 * Math.PI * frequency * time + noteIndex * .07), 0) * .008 * padMotion;
      if (style !== 'hotel-ambient') {
        ...
        value += (Math.sin(2 * Math.PI * frequency * time) + .22 * Math.sin(4 * Math.PI * frequency * time)) * .028 * envelope;
      }
      samples[index] = Math.max(-.2, Math.min(.2, value));
    }
    return buffer;
  }
    ...
    try {
      ...
      const drawFrame = (now) => {
        try {
          ...
          drawEmphasis(ctx, item.text, item.sceneTag || state.assets[item.assetIndex]?.sceneType, width, height, localProgress);
          drawCaption(ctx, item.text, width, height, localProgress);
