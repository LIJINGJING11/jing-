...
//
// Demo 经常会出现一个旧的静态页面（例如 4173）和一个真正的模型服务（4174/4175）
// 同时打开的情况。不要把接口地址永远锁死在当前页面的相对路径上，否则旧页面会只显示
// 浏览器原生的 “Failed to fetch”。在本机 Demo 中按顺序尝试当前页面、4174、4175。
const MODEL_GENERATE_ENDPOINT = window.HOTEL_MODEL_ENDPOINT || '';
const MODEL_SERVICE_PORTS = [4174, 4175, 4176, 4177, 4178];
...
// 打开的 file:// 页面，也支持从 4174-4178 任一端口启动的本地服务。
...
const apiConfigState={provider:'company',configured:false,model:'gpt-image-2',apiBaseUrl:'https://coding.efficient.center/api/v1',videoModel:'doubao-seedance-2-5-260628',videoApiBaseUrl:'https://live-turing.cn.llm.tcljd.com/api/v1',audioModel:'turing/tts-1',actorAuthorization:'image-generation',quality:'medium',endpoint:''};
...
function applyApiConfigToForm(status={}){
  ...
  const provider=$('#apiProviderInput'),model=$('#apiModelInput'),base=$('#apiBaseUrlInput'),videoModel=$('#apiVideoModelInput'),videoBase=$('#apiVideoBaseUrlInput'),audioModel=$('#apiAudioModelInput'),actor=$('#apiActorInput');
...
async function saveApiConfigFromForm(){
  ...
  const visionModel=$('#apiVisionModelInput')?.value.trim()||'';
  ...
  if(!visionModel){setApiConfigMessage('请填写支持图片输入的场景识别视觉模型 ID。',true);$('#apiVisionModelInput')?.focus();return}
  ...
  try{
    const status=await postApiConfig({provider,apiKey,model,apiBaseUrl,videoModel,videoApiBaseUrl,audioModel,visionModel,actorAuthorization});
    applyApiConfigToForm(status);setApiConfigMessage(`图片、视频和音频接口已保存；共用同一把 API Key。`);
...
async function clearApiConfigFromForm(){
  ...
  try{const status=await deleteApiConfig();applyApiConfigToForm(status);const key=$('#apiKeyInput');if(key)key.value='';setApiConfigMessage('已清除当前运行时配置。重新启动服务后，启动脚本中的钥匙串配置仍可继续使用。')}catch(error){setApiConfigMessage(friendlyApiConfigError(error),true)}finally{if(button)button.disabled=false}
}
