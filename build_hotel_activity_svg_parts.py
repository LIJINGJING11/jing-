...
def data(name):
    return 'data:image/png;base64,' + base64.b64encode((ASSET / name).read_bytes()).decode('ascii')
...
def wrap(name, body):
    out = ROOT / f'hotel-activity-{name}.svg'
    out.write_text(
        f'<svg xmlns="http://www.w3.org/2000/svg" width="960" height="1706" viewBox="0 0 960 1706">'
        f'<g id="{name}" data-layer-type="frame">{body}</g></svg>',
        encoding='utf-8'
    )
    print(out)
